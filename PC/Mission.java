...
public class Mission {
	...
	public void addHospital(int x, int y) {
		...
		gridArray[x][y] = 'H';
		locationList.add(0, new Location("H", x, y));
	}
	...
	public void addPotentialVictim(int x, int y) {
		...
		locationList.add(new Location(locationNames[locationNameIndex], x, y)); 
		locationNameIndex++;
	...
	public void foundNonCritical(int x, int y) {
		...
		if (unvisitedPotentialLocations == 0) {
			...
			pf.goToHospital();
			actualVictimsRemaining--;
			pf.rescueRemainingNonCriticals(actualVictimsRemaining);
		}
		else {
			pf.goToNextVictim();
		}
