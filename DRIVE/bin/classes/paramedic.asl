...
+startRescueMission(D,C,NC) : location(hospital,X,Y) & 
    ...
    <- .count(location(victim,_,_),Vcount);		// Determine the victims
       ...
       .print("Start the Resuce mission for ",C," critical and ",NC,
    		...
    		Vcount, " victimes and ", Ocount," known obstacles"); finished(true).
...
// doctor4 agents, via the achievement task +!requestVictimStatus(D,X,Y,C)

+critical(X,Y)
       //removes belief of victim existing in doctor as it will be taken to the hospital straight away
	<- .abolish(location(victim, X , Y));
       .print("The victim at ", X, ",", Y, " is critical"); critical(X,Y).
	   

+~critical(X,Y)
    <- .print("The victim at ", X, ",", Y, " is not critical"); nCritical(X,Y).


// ========================================================================
// Plan Library for achievement goals 
// ========================================================================
// Request the beliefs of the locations.  D is the doctor4 agent
+!getScenario(D) <- .send(D,askAll,location(_,_,_)).
	
// Check the status of a victim
// D is the doctor4 agent, X, Y are cell coordinates, and
// C is a colour from the set {burgandy,cyan}
// If the doctor4 knows the colour, it will send a belief for the 
...
+remove_location(X,Y)
	<- .abolish(location(victim, X , Y)); nVictim(X,Y). 
